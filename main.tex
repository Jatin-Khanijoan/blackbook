...
Submitted in partial fulfilment of the requirements of the degree of \\[0.5cm]

BACHELOR OF COMPUTER ENGINEERING \\[0.5cm]

\textbf{by} \\[0.3cm]
Manas Nanivadekar - 22102033 \\
Jatin Khanijoan - 22102094 \\
Swayam Kothekar - 22102047\\
Amaan Khan - 22102004\\[1cm]

Guide: \\[0.3cm]
Prof. Deepak S. Khachane \\[1cm]
...
\begin{spacing}{1.5}
\noindent This is to certify that the project entitled \textbf{``End-to-End Autonomous Decision-Making Framework: Integrating Information-Directed Conformal Retrieval and Factored Optimal Transport Reward Shaping''} is a bonafide work of \textbf{Manas Nanivadekar (22102033), Jatin Khanijoan (22102094), Swayam Kothekar (22102047), Amaan Khan (22102007)} submitted to the University of Mumbai in partial fulfilment of the requirement for the award of the degree of \textbf{Bachelor of Engineering} in \textbf{Computer Engineering}.
\end{spacing}
...
\begin{spacing}{1.5}
\noindent This project report entitled \textbf{``End-to-End Autonomous Decision-Making Framework: Integrating Information-Directed Conformal Retrieval and Factored Optimal Transport Reward Shaping''} by \textbf{Manas Nanivadekar, Jatin Khanijoan, Swayam Kothekar, Amaan Khan} is approved for the degree of \textbf{Bachelor of Engineering} in \textbf{Computer Engineering}, 2025-2026.
\end{spacing}
...
\vspace{0.5cm}
Over the last few years, making reliable decisions in high-stakes domains such as finance and medicine has become increasingly reliant on artificial intelligence. While existing AI models can rapidly process vast amounts of data, their decision-making processes often suffer from overconfidence, outdated information, and a lack of verifiable reasoning. Traditional retrieval systems attempt to solve this by fetching related documents based on semantic similarity, rather than selecting information actually required to resolve uncertainty. This project presents an autonomous multi-agent decision-making framework that actively seeks out the most critical data needed to make sound, mathematically calibrated decisions. The proposed framework operates as a unified, end-to-end system that dynamically combines adaptive information gathering with collaborative decision-making. By modeling document retrieval as a process of continuous uncertainty reduction, the framework ensures that the AI forms highly verifiable recommendations. Utilizing specialized collaborative agents to evaluate dynamic financial environments, the application drastically improves algorithmic portfolio allocation and mitigates catastrophic risk. This uncertainty-aware pipeline represents a significant leap forward, offering a robust template for the future of automated decision analysis.
...
\begin{center}
\textbf{\Large Abbreviations and Nomenclature}
\end{center}
...
\vspace{1cm}
\noindent\textbf{Greek Symbols and Notation:}

\begin{longtable}{p{0.18\textwidth} p{0.75\textwidth}}
$\alpha$        & Significance level (miscoverage rate) in conformal prediction \\
$\Sigma$        & Posterior covariance matrix \\
$\Sigma_0$      & Prior covariance matrix (initial precision $\Lambda_0$) \\
$\Lambda$       & Precision matrix ($\Lambda = \Sigma^{-1}$) \\
$\Theta$        & Asset return precision matrix (graphical lasso estimate) \\
$\rho^\pi_{ij}$ & Pairwise marginal occupancy measure of agents $i, j$ under policy $\pi$ \\
$\rho^*_{ij}$   & Target pairwise occupancy measure on edge $(i,j)$ \\
$\lambda$       & FOTRS shaping coefficient; also graphical lasso penalty \\
$W_{c_{ij}}$    & Wasserstein distance with cost $c_{ij}$ on the $(i,j)$ factor \\
$\mathcal{N}(i)$& Neighbourhood of agent $i$ in the coordination graph \\
$g(S)$          & IDCR submodular gain function: $\log\det\Sigma_0^{-1} - \log\det\Sigma(x,S)$ \\
\end{longtable}
...
This structured process appears consistently across a wide range of disciplines. For example, in financial decision making, an investor evaluates market conditions, gathers financial indicators and research reports, analyzes possible investment strategies, and finally selects an allocation that best satisfies their risk and return objectives. Similarly, in medical diagnosis, clinicians assess a patient's current symptoms, consult clinical guidelines and medical knowledge, evaluate possible diagnoses, and determine an appropriate treatment plan.

The increasing availability of computational power and artificial intelligence techniques has created opportunities to automate parts of this reasoning process. Modern AI systems, particularly large language models, are capable of analyzing textual information across multiple domains and generating structured responses to complex queries. However, designing systems that can reliably perform structured decision making remains a significant challenge.

In particular, real-world autonomous decision-making demands robust mathematical formulations to manage uncertainty and multi-agent coordination. Portfolio management at an institutional scale, for instance, is fundamentally a coordination problem. Different desks handle different sectors or asset classes, each possessing localized market information, but the global portfolio is evaluated on centralized metrics such as the Sharpe ratio. Moving to the algorithmic level exposes the classic Multi-Agent Reinforcement Learning (MARL) credit assignment bottleneck: when a global performance metric underperforms, assigning the precise fault to an individual agent (or sector) becomes mathematically intractable using conventional scalar reward distributions. 
...
However, practical deployment of such systems reveals several critical limitations. Many AI models operate primarily by recognizing semantic patterns from their training data rather than by performing structured reasoning. As a result, their outputs may appear highly confident even when the underlying information is incomplete, ambiguous, or outdated. In domains such as finance, medicine, or law, this limitation can lead to unreliable conclusions if the system cannot verify its reasoning against reliable external information. Retrieval-augmented generation (RAG) grounds language models in external evidence, yet standard pipelines rank documents purely by embedding similarity or BM25, and then synthesize a response. This creates an objective mismatch: the retrieval criterion (semantic relevance) is entirely decoupled from the downstream goal (predictive accuracy under calibrated uncertainty).

Another challenge arises from the dynamic nature of real-world knowledge. Financial markets evolve continuously, medical guidelines are frequently updated, legal regulations change over time, and scientific understanding expands with new research findings. Systems that rely solely on previously trained knowledge may therefore produce answers that do not reflect the most current information. A document that is semantically close to the query may be redundant with information already retrieved, whereas a seemingly distant piece of information might decisively reduce uncertainty across multiple candidate outcomes simultaneously. What fundamentally matters for robust decision systems is not semantic similarity, but precisely how much each optimally retrieved document analytically reduces predictive uncertainty.

Furthermore, complex decisions often require integrating multiple perspectives rather than relying on a single analytical model. In many professional environments, experts from different specializations collaborate to reach a final conclusion. Replicating this multi-perspective reasoning process within an automated system presents an immense challenge for the design of intelligent decision-support frameworks. Conventional MARL methods either hand every agent the exact same global reward---which inevitably induces free-riding and sluggish learning dynamics---or attempt to train a centralized critic over the full joint state-action space, which scales exponentially with the number of agents. Unlocking verifiable, autonomous decision-making requires dismantling these multi-agent bottlenecks.

These structural shortfalls motivate the development of a more mathematically grounded approach to autonomous decision making. The proposed system aims to address these limitations by integrating submodular information-directed retrieval, optimal transport-based multi-agent reasoning, distribution-free conformal uncertainty modeling, and adaptive learning mechanisms into a unified framework capable of producing mathematically reliable and transparent decision recommendations.

\section{Aim of the Project}

The primary aim of this project is to design and implement an end-to-end autonomous decision-making framework capable of generating calibrated, structured recommendations for complex real-world problems.

The system aims to automate the fundamental stages of the decision-making process, including identifying the current problem state using Markov models, retrieving relevant information sources by minimizing downstream conformal prediction set volumes, mapping those resources to possible decisions through coordination graphs, and evaluating the outcomes via factored optimal transport dynamics. By integrating profound theoretical advances such as entropy-regularized Sinkhorn distances, log-determinant submodular maximization, and potential games, the proposed framework strictly ensures improved policy alignment towards Nash equilibria, significantly improving the reliability and interpretability of autonomous AI-driven decision support systems.
...
\noindent Beketov et al.\ \cite{Beketov2018RoboAdvisory} survey the quantitative methods underlying commercial robo-advisor platforms, covering mean-variance optimisation, Black-Litterman models, factor models, and risk parity strategies. They assess the gap between academic optimality and practical implementability under real-world constraints. This survey benchmarks the robo-advisory approach the FOTRS portfolio agent supersedes.
...
95 & Robo advisors: Quantitative methods inside the robots \cite{Beketov2018RoboAdvisory} & Beketov, M., et al. \newline\newline \textit{Journal of Asset Management, 19, 2018} & Survey of robo-advisor quantitative methods; gap between academic optimality and practical constraints assessed. \\ \hline

96 & The promises and pitfalls of robo-advising \cite{DacuntoRoboadvisor2019} & D'Acunto, F., et al. \newline\newline \textit{Review of Financial Studies, 32(5), 2019} & Robo advisors improve efficiency for some clients but fail to handle tax situations and behavioural prefs. \\ \hline

97 & Prospect theory: An analysis of decision under risk \cite{Kahneman1979ProspectTheory} & Kahneman, D., \& Tversky, A. \newline\newline \textit{Econometrica, 47(2), 1979} & Foundational behavioural finance model; loss aversion and probability weighting explain irrational choices. \\ \hline

98 & Mental accounting and consumer choice \cite{Thaler1985MentalAccounting} & Thaler, R. H. \newline\newline \textit{Marketing Science, 4(3), 1985} & Non-fungible psychological accounts distort rational wealth decisions; explains disposition effect. \\ \hline
...
108 & Mastering Go with deep neural networks and tree search (AlphaGo) \cite{Silver2016AlphaGo} & Silver, D., et al. \newline\newline \textit{Nature, 529, 2016} & DRL + MCTS achieves superhuman Go performance; proof-of-concept for deep RL in complex strategy domains. \\ \hline

109 & Dynamic LLM-agent network with agent team optimisation \cite{Liu2023LLMAgents} & Liu, X.-Y., et al. \newline\newline \textit{arXiv:2310.02170, 2023} & Evolutionary agent team optimisation; heterogeneous teams with adaptive communication outperform homogeneous. \\ \hline

110 & Reinforcement learning: An introduction (2nd ed.) \cite{Sutton2018RL} & Sutton, R. S., \& Barto, A. G. \newline\newline \textit{MIT Press, 2018} & Canonical RL textbook; MDP, TD learning, and policy gradient foundations for all RL components used. \\ \hline
...
\section{Objectives}
The primary objectives of this project form a comprehensive technical pipeline defined as follows:
\begin{enumerate}
    \item To design a rigorous autonomous decision-making framework mathematically capable of analyzing complex, non-stationary problem topologies across multi-dimensional domains.
    \item To develop a formalized approach for Information-Directed Conformal Retrieval (IDCR) that selects subset constraints of document corporas by exploiting the properties of submodular maximization and monotonic covariance log-determinants, strictly targeting uncertainty reduction over pure textual relevance.
    \item To algorithmically integrate split-conformal prediction techniques that output probabilistically guaranteed prediction sets, allowing the system to express rigorously calibrated $1-\alpha$ confidence manifolds in its empirical recommendations.
    \item To architect a Multi-Agent Markov Decision Process (MMDP) reasoning structure governed by Factored Optimal Transport Reward Shaping (FOTRS), where decentralized specialized agents evaluate local factored Wasserstein distances, bypassing exponential joint-space complexity and eliminating the global credit assignment distortion.
    \item To theoretically prove and empirically evaluate the performance scalability of the proposed framework, measuring greedy approximation bounds against classical exhaustive search, and charting optimal transport gradient convergence rates against baseline MARL algorithms including QMIX and MAPPO.
\end{enumerate}

\section{Scope of the Project}
The operational and theoretical scope of this encompassing project fundamentally includes the following aspects:
\begin{enumerate}
    \item The framework strictly targets autonomous decision-making algorithms, designing an end-to-end mathematical construct that organically binds information-theoretic submodular retrieval, continuous distribution-free conformal modeling, and localized optimal transport multi-agent negotiations across complex domains like quantitative finance and precision clinical diagnostics.
    \item The operational development strongly emphasizes topological coordination graphs, derived from asset precision matrices utilizing graphical lasso regularizations, establishing structural limits and exact potential game parameters to drive completely decentralized policy updates.
    \item Experimental algorithmic evaluation strictly focuses on verifying theoretical constraints, comparing the $\approx 63\%$ submodular approximation bound against intractable brute-force searches, evaluating empirical nonconformity score coverages, and observing exact Sharpe ratio accelerations over historical market regimes.
    \item Commercial deployment parameters, extreme high-frequency execution latency optimizations beneath the millisecond domain, specific low-level systemic hardware scaling (such as multi-node cluster load balancing), and domain-specific regulatory legislative compliance frameworks remain specifically abstract and lie beyond the core algorithmic scope of this theoretical implementation.
\end{enumerate}

\vspace{1em}
\noindent Table~\ref{tab:objectives_summary} summarizes the five primary technical objectives of the proposed framework along with their corresponding algorithmic mechanisms and verifiable success criteria.

\begin{table}[h!]
\centering
\renewcommand{\arraystretch}{1.4}
\caption{Summary of Project Objectives, Mechanisms, and Success Criteria}
\label{tab:objectives_summary}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|c|p{5cm}|p{5.5cm}|p{5cm}|}
\hline
\textbf{Obj.\#} & \textbf{Objective} & \textbf{Mechanism} & \textbf{Success Criterion} \\
\hline
1 & Autonomous multi-domain decision framework & End-to-end IDCR + FOTRS pipeline & Validated on finance and clinical domains \\ \hline
2 & Information-Directed Conformal Retrieval & Submodular log-det covariance maximization & $\geq 4\times$ CP volume reduction vs random \\ \hline
3 & Calibrated uncertainty quantification & Split-conformal prediction sets at $1-\alpha$ & Empirical coverage $\in [1-\alpha-0.07, 1-\alpha+0.07]$ \\ \hline
4 & Factored optimal transport reward shaping & MMDP + Wasserstein coordination graph & Sharpe Ratio $\geq 0.85$ (out-of-sample) \\ \hline
5 & Scalability and convergence guarantees & Greedy $1-1/e$ bound + Nash equilibrium proof & Convergence in $< 80$ training episodes \\ \hline
\end{tabular}%
}
\end{table}
...
While traditional, monolithic artificial intelligence systems—particularly those relying on naive prompt engineering or unstructured generative loops—often dangerously attempt to hallucinate answers directly from static, interpolated learned patterns or disconnected, out-of-date static knowledge bases, the proposed mathematical framework explicitly focuses on rigorously mathematically modeling the continuous decision-making process itself as a dynamic geometric optimization problem. Rather than dangerously treating complex reasoning as a single, uninterpretable black-box interpolation step, the proposed system fundamentally and structurally decomposes the overarching decision-making cognitive process into multiple, highly specialized, sequentially coordinated mathematical stages that collectively and rigorously contribute converging gradient updates to the final precisely bounded systemic outcome.
...
The comprehensively designed proposed framework is analytically and conceptually seamlessly applicable across an incredibly vast array of highly complex, heavily constrained decision-making environments, including decentralized quantitative algorithmic finance, precision medical diagnostics, rigorous contract law, and purely theoretical empirical scientific analysis methodologies. However, the exact technical implementation, rigorous bounds formulation, and exhaustive experimental metric evaluation specifically presented mathematically within this specific project exclusively focus specifically upon high-frequency, complex financial decision-making executed strictly within the highly volatile continuous context of the stochastic Indian stock market structural matrices. In this highly constrained mathematical setting, the completely autonomous system successfully and reliably generates optimized, strictly bounded sector-wise continuous portfolio allocation fractional recommendations relying entirely unequivocally upon complex investor profile multi-dimensional tensor information strictly coupled with continuously actively retrieved unstructured financial reporting documents.

\section{Architecture Diagram}

An Architectural diagram is a visual representation that maps out the physical implementation of the components of the software system. The overall architecture of the end-to-end autonomous decision-making framework illustrates the flow from user input to the final conformal portfolio allocation.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Styles
        base/.style={draw, thick, align=center, inner sep=2ex, drop shadow},
        user/.style={base, circle, fill=blue!10, text width=2.5cm, minimum size=3cm},
        module/.style={base, rectangle, rounded corners, fill=white, text width=4cm, minimum height=1.5cm},
        engine/.style={base, rectangle, rounded corners, fill=white, text width=4.5cm, minimum height=2.5cm},
        database/.style={base, cylinder, shape border rotate=90, aspect=0.25, fill=yellow!20, text width=2.5cm, minimum height=2.5cm},
        group/.style={draw, thick, dashed, rounded corners, inner sep=4ex, fill=gray!5},
        arrow/.style={->, thick, rounded corners},
        bidir/.style={<->, thick, rounded corners}
    ]

    % Nodes
    \node[user] (user) {\textbf{User / Investor}};

    \node[module, right=3.5cm of user, yshift=2.5cm] (qi) {\textbf{Query Interpreter}\\[0.5em] \footnotesize Parses financial profile and queries};

    \node[module, below=1cm of qi] (idcr) {\textbf{IDCR Module}\\[0.5em] \footnotesize Uncertainty-reducing submodular retrieval};

    \node[database, right=2.5cm of idcr] (kb) {\textbf{Financial}\\\textbf{Knowledge Base}\\[0.5em] \footnotesize Documents, Markets};

    \node[engine, below=1cm of idcr] (fotrs) {\textbf{FOTRS Engine}\\[0.5em] \footnotesize \textit{Banking, IT, Pharma...}\\[0.5em] \footnotesize Multi-Agent Negotiation};

    \node[module, below=1cm of fotrs] (cp) {\textbf{Conformal Predictor}\\[0.5em] \footnotesize Calibrates prediction set};

    \node[user, fill=green!10, left=3.5cm of cp] (output) {\textbf{Portfolio}\\ \textbf{Allocation}};

    % Background Group (System Core)
    \begin{scope}[on background layer]
        \node[group, fit=(qi) (idcr) (fotrs) (cp), label={[font=\sffamily\bfseries\large, yshift=2ex]above:End-to-End Autonomous Decision System}] (core) {};
    \end{scope}

    % Edges
    \draw[arrow] (user.east) -- node[above, font=\footnotesize, sloped] {Financial Profile} node[below, font=\footnotesize, sloped] {\& Queries} (qi.west);

    \draw[arrow] (qi.south) -- node[right, font=\footnotesize] {System State ($x$)} (idcr.north);

    \draw[bidir] (idcr.east) -- node[above, font=\footnotesize, text width=2cm, align=center] {Retrieval\\Requests} node[below, font=\footnotesize, text width=2cm, align=center] {Facts} (kb.west);

    \draw[arrow] (idcr.south) -- node[right, font=\footnotesize] {Context ($S$)} (fotrs.north);

    \draw[arrow] (fotrs.south) -- node[right, font=\footnotesize] {Joint Decisions ($\hat{y}$)} (cp.north);

    \draw[arrow] (cp.west) -- node[above, font=\footnotesize] {Statistically Guaranteed} node[below, font=\footnotesize] {Prediction Set $C_\alpha(x)$} (output.east);

    \draw[arrow, dashed, color=gray] (output.north) -- node[left, font=\footnotesize, text width=2cm, align=right] {Actionable\\Insights} (user.south);
...
This confirms that $\Phi$ satisfies the exact potential game condition. $\checkmark$
...
\]
Since Wasserstein distances are non-negative ($W_{c_{ij}} \geq 0$), the second sum is non-negative, establishing the lower bound. $\checkmark$
...
&\leq W_{\text{fact}}(\rho^\pi, \rho^*; T) + \sum_{(i,j) \in E \setminus T} w_{ij} D_{ij}. \qquad \checkmark
\end{align*}
...
To service this query, the Autonomous Decision System issues a \emph{Document Retrieval Request} to the Financial Knowledge Base API. This external entity encompasses a curated, continuously updated repository of structured financial data---including sectoral index time-series, macroeconomic indicators, central bank policy announcements, FII transaction records, and earnings disclosures---as well as an unstructured corpus of domain-specific textual documents indexed as 384-dimensional dense vector embeddings. The Knowledge Base returns a \emph{Candidate Document Set} to the system, which represents the pool of potentially relevant evidence over which the submodular retrieval optimization will subsequently operate.

Upon completing its internal processing pipeline---encompassing conformal retrieval, multi-agent reasoning, and prediction set calibration---the system returns two output flows to the Investor. The primary output is the \emph{Conformal Portfolio Allocation Set} $C_\alpha(x)$: a formally guaranteed, finite-sample prediction region over the portfolio weight simplex, containing the set of all allocation decisions consistent with the investor's constraints at the target coverage level $1-\alpha$. The secondary output is a \emph{Confidence Audit Report}, providing an interpretable summary of the information sources that drove the allocation recommendation, enabling human-in-the-loop verification and regulatory compliance documentation.
...
\textbf{Message 9 -- Allocation Display:} Finally, the System Interface renders the conformal prediction set as a structured portfolio allocation report and returns the \emph{Display Allocation} response to the Investor as a dashed message. This response includes the recommended weight intervals for each sector, the empirical coverage guarantee level $1-\alpha$, and the confidence audit provenance report.

\section{Activity Diagram}

An Activity Diagram illustrates the dynamic aspects of the system by depicting the flow of control from one activity to another. It visualizes the step-by-step workflow of the portfolio allocation algorithm.
...
The implementation of the system relies on the following core technologies and their specific versions to ensure reproducibility and stability:
\begin{itemize}
    \item \textbf{Programming Language:} Python 3.10
    \item \textbf{Deep Learning Framework:} PyTorch 2.1.0 
    \item \textbf{Reinforcement Learning Library:} Ray RLlib 2.8.0 
...
The experiments, including the multi-agent reinforcement learning training loops and the continuous document vector indexing, were conducted on a high-performance workstation with the following specifications:
...
\hline
\multirow{6}{*}{Software} & Language & Python 3.10 \\
 & DL Framework & PyTorch 2.1.0 \\
 & RL Library & Ray RLlib 2.8.0 \\
 & Scientific Stack & NumPy 1.26 / SciPy 1.11 / Pandas 2.1 \\
...
\centering
\renewcommand{\arraystretch}{1.4}
\caption{Evaluation Metrics, Baseline Systems, and Measured Dimensions}
\label{tab:eval_metrics}
\small
\begin{tabular}{|p{3.2cm}|p{3.2cm}|p{7.5cm}|}
\hline
\textbf{Metric} & \textbf{Applicable Module} & \textbf{Description} \\
\hline
Sharpe Ratio & FOTRS (Portfolio) & Risk-adjusted annualized return \\ \hline
Maximum Drawdown & FOTRS (Portfolio) & Peak-to-trough loss during crisis regimes \\ \hline
CP Set Volume & IDCR (Retrieval) & Geometric volume of conformal prediction ellipsoid \\ \hline
Empirical Coverage & IDCR (Retrieval) & Fraction of true labels covered at level $1-\alpha$ \\ \hline
Retrieval Latency (ms) & IDCR (Retrieval) & Mean query processing time per document subset \\ \hline
Convergence Episodes & FOTRS (MARL) & Episodes to reach Nash equilibrium \\ \hline
Spearman $\rho_s$ & FOTRS (Credit) & Correlation of OT distance to realized crisis losses \\ \hline
\end{tabular}
\end{table}
\vspace{0.5em}
\noindent The competing baseline systems are: (i) Zero-Shot GPT-4o prompting, (ii) Tool-Augmented LangChain prompting, (iii) Standard cosine-similarity RAG, (iv) Cross-encoder reranker, (v) MARL-QMIX, and (vi) MARL-MAPPO.
...
\begin{figure}[h]
    ...
    \caption{Marginal gain as a function of retrieval steps. Gains diminish with increasing steps, highlighting diminishing returns, especially compared to the synergistic corpus baseline.}
...
\subsection{Profound Cross-Domain Generalization Boundaries and Analytical Precision Tradeoffs}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\linewidth]{figures/fig3.png}
    \caption{Performance gap percentage across different corpus types. The largest gap is observed in synergistic corpora, indicating higher complexity and interaction effects.}
    \label{fig:gap_corpus}
\end{figure}

Dramatically expanding algorithmic evaluations while stretching parameter verification beyond strictly defined clinical boundaries, the system evaluated parameters on GoEmotions multi-label mapping sets with $N=2000$. Results revealed a mathematically significant Pareto tradeoff boundary: algorithmic baseline limits achieved peak F1 metrics reaching $0.201$, yet failed to satisfy the predefined 90\$ statistical coverage constraint, reaching only $0.888$ while generating large spatial prediction variance regions across boundary mapping matrices. Alternatively, the IDCR formulation maintained structural analytical limits, consistently reporting $\geq 0.94$ empirical spatial coverage while compressing geometric boundary volumes by over $33\times$. This compression preserved metric scaling across bounded dimensions while significantly reducing spatial footprint. This demonstrates that optimizing explicitly for lexical F1 accuracy degrades quantitative calibration and structural parameter alignment, confirming a fundamental tradeoff between accuracy optimization and bounded statistical coverage limits.
...
Standard RAG & 0.51 & 48.6 & $1.1\times$ & 0.870 & -- \\ \hline
MARL-QMIX & 0.68 & 43.2 & -- & -- & 210 \\ \hline
MARL-MAPPO & 0.73 & 41.0 & -- & -- & 185 \\ \hline
Risk Parity & $-0.35^*$ & 67.3 & -- & -- & -- \\ \hline
\textbf{IDCR + FOTRS (Proposed)} & \textbf{0.89} & \textbf{34.0} & $\mathbf{4.8\times}$ & \textbf{0.943} & \textbf{78} \\ \hline
\end{tabular}%
}
{\small $^*$ Crisis regime only (bull-market Sharpe = 0.74).}
\end{table}

As shown in Table~\ref{tab:results_summary}, the proposed FOTRS engine achieved an unprecedented Sharpe ratio of \textbf{0.89}---a monumental $22\%$ improvement over MAPPO (0.73). Maximum drawdown was structurally restricted to merely 34.0\%, versus the 41.0\% failure during MAPPO execution. The IDCR module simultaneously reduced conformal prediction set volumes by $4.8\times$ relative to random retrieval, while maintaining empirical coverage of $0.943$ at $\alpha=0.10$.
 Crucially, maximum drawdown risk constraints were rigidly and structurally restricted to merely 34.0\% over extreme market cycles, demonstrating vast algorithmic superiority over the 41.0\% drawdown failure observed during MAPPO execution tests. Furthermore, by deliberately isolating the algorithmic graph factorization constraints during aggressive ablation studies, the mathematical necessity of the graphical lasso architecture was completely validated: an unfactored, traditional Wasserstein distance shaping baseline completely failed to coordinate the agents, stalling at a mediocre 0.72 Sharpe ratio. This structural demonstration definitively proves that actively decomposing highly dense rewards explicitly and precisely over the mathematically determined theoretical coordination graph edge boundaries constitutes the exact functional mechanism mathematically preventing catastrophic joint-policy collapse.

\subsection{Regime-Specific Risk Dynamics and Direct Credit Assignment Validation}
During explicitly modeled extreme market crisis regimes and heavy volatility phase shifts, the comparative architectural difference was entirely unprecedented and statistically profound: traditional financial Risk Parity optimization deteriorated violently into an unacceptable $-0.35$ absolute continuous Sharpe ratio, whereas the intelligent FOTRS architecture independently inverted aggressive market structural forces to actively and safely achieve a positive $+0.12$ Sharpe execution boundary. Notably, FOTRS stood completely alone as the singular positive-yielding algorithm evaluated across the entirety of the crisis simulations. This profound systemic resilience is fundamentally and analytically rooted entirely within the optimal mathematical transport early-warning structural penalty signals generated internally. 
...
\begin{figure}[h]
    ...
    \caption{Sample efficiency comparison showing the number of training episodes required as the number of agents increases. FOTRS demonstrates significantly better scalability than MAPPO and QMIX.}
...
\begin{figure}[h]
    ...
    \caption{Comparison of Sharpe ratio performance across training episodes for FOTRS, MAPPO, and QMIX. FOTRS achieves faster convergence and higher stability compared to baseline methods.}
    \label{fig:sharpe_vs_episodes}
\end{figure}
...
The research and development work from this project has resulted in the following publications:

\begin{enumerate}
    
    \item \textbf{Title:} ``Factored Optimal Transport Reward Shaping for Multi-Agent Portfolio Allocation''
    \begin{itemize}
        \item \textbf{Authors:} Manas Nanivadekar, Jatin Khanijoan, Swayam Kothekar, Amaan Khan, Prof. Deepak Khachane
        \item \textbf{Venue:} ACM International Conference on AI in Finance (ICAIF) 2026.
        \item \textbf{Status:} Submitted / Accepted
    \end{itemize}
\end{enumerate}
